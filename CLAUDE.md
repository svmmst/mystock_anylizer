...
### 2026-07-04：market_regime.py 增加"数据日期校验与提醒"

> 本次改动本应在 mystock_anylizer 侧规划，实际由 real_stock 侧的实盘复盘排查触发并完成，故在此补记。备份见 `market_regime.py.bak-20260704`。

**背景**：`market_regime.py` 所有取数用 `WHERE date <= ? ORDER BY date DESC LIMIT N`，即"取不晚于请求日期的最近 N 条"。当请求日期在库里无数据时，会**静默回退到更早的最近一条**，但输出 `"date"` 仍显示请求日期——用旧行情冒充请求日且无提示。历史日期回测/复盘不受影响（库里有数据正好取到当天），风险只在实盘取当天、数据尚未入库或两表更新不同步时。（实证：7/2 曾因指数表 bug 导致指数趋势维度失真，评分/档位判错，见 real_stock 日志勘误。）

**改动（仅 market_regime.py，评分算法/权重/阈值/分类/仓位映射全部未动，向后兼容）**：
1. 四个维度评分函数各返回 `actual_date`（该维度实际取到的最新数据日期）；`score_index_trend` 用指数表日期、`score_market_breadth` 用个股表 `daily_price_qfq` 日期。
2. `run_regime` 汇总校验：以市场宽度（个股表）实际日期为**数据基准日 `data_date`**；若 `data_date != 请求日` 或"个股表日期 ≠ 指数表日期"，生成 `data_warning` 醒目提醒。
3. 输出：`"date"` 改为反映**真实数据日期**（回退时即实际行情日），新增 `requested_date` / `data_date` / `data_warning`，各维度带 `actual_date`。文本报告用 `!!!!` 框突出告警。
4. **默认日期（不传 `--date`）改为取 `daily_price_qfq` 与 `index_daily_price` 两表 MAX(date) 的较小值（min）**——保证四个维度落在"两表都有数据"的同一交易日，避免个股表已更、指数表未更时维度间日期打架。宁用稍旧但完整的一天，不用最新但残缺的一天。

**行为变化**：请求日无数据时**仍照常跑出结果**（不报错退出），但会如实显示实际行情日并醒目提醒"基于哪天数据"。下游（real_stock 通过 `--json` 读取）原有字段全部保留。
